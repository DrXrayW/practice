 ...
 * [adjustments]
 * 
 * NOTE that the node with left sub tree is visited twice, so we cannot simply output node first
 * 
 * When there is no left subtree, root go first, so nothing changes here.
 * 
 * There is no need to wait for the left subtree to finish, instead, we want to output the node before going to left tree
 * 
 * IN SHORT: 没左，输出，走右；有左，找前继，记链接，输出！，走左；如前继标记自己，恢复，走右
 * 
 */

public class BinaryTreePreorderTraversal_Morris {
